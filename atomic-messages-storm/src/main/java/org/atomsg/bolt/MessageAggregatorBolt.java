...
public class MessageAggregatorBolt implements IRichBolt, IMessageAggregatorBolt {

	private static final long serialVersionUID = 1L;
	private OutputCollector collector;
	private Map<String, Object> config;
    private static final Logger log = Logger.getLogger(MessageAggregatorBolt.class);
	private MessageAggregatorBoltLogic logic = new MessageAggregatorBoltLogic();
    ...
    public void execute(Tuple tuple) {
    	anchor = tuple;
	    try {
	    	if ("AtomicMessages".equals(tuple.getSourceStreamId())) {
	    		Message message = Marshaller.asMessage(tuple);
	            logic.readFromAtomicMessages(message, this);
	    	}
		} catch (Exception e) { 
			log.error("MessageAggregatorBolt execute() error", e); 
	    }
	}
